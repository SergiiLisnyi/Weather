...
extension ModelWeatherProtocol  {
    ...
    func getWeatherOnFiveDay(keyCity: String, updateScreen: @escaping (String?)->())  {
    ...
    func getWeatherOnHourly(city: String, keyCity: String, updateScreen: @escaping (String?)->())  {
        let url = ApiData.BASE_URL + "hourly/12hour/" + keyCity + "?apikey=" + ApiData.APIKEY + "&metric=true"
        Request.requestWithAlamofire(url: url, complete: { data, error in
            self.weatherOnDay = ForecastWeatherDay(tempCurrent: data[0]["Temperature"]["Value"].description + "°")
